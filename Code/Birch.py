...
class BirchChunked(Birch):
    @profile
    #modified predict method to break up the dataset into chunks and do calculations on them
    def predict(self, X):
        # the original code
...
class NewBirch(Birch):
    @profile
    #modified predict method to help with matrix multiplication as the old birch was showing memory errors
    def predict(self, X):
        X = check_array(X, accept_sparse='csr')
        self._check_fit(X)
...
# and plot.
birch_models = [NewBirch(threshold=1.7, n_clusters=None)
                ,BirchChunked(threshold=1.7, n_clusters=None)

                ]
final_step = [ 'with new predict(X) method' 
              ,'with chunked predict(X)'
